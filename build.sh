...
if [ -d "${CURRENT_PATH}/proto/$1" ];then
  rm -rf "${CURRENT_PATH}/golang_pb/$1"
  mkdir -p "${CURRENT_PATH}/golang_pb/$1"
  cp -rf ${CURRENT_PATH}/proto/$1/*.proto "${CURRENT_PATH}/golang_pb/$1/"
  cd "${CURRENT_PATH}/golang_pb/$1" || exit 1
  ...
  rm -rf *.proto
  # 修改 go 文件中的 json tag
  cd $CURRENT_PATH && protoc-go-inject-tag -input=./golang_pb/blood/v1/*.pb.go
else
  echo "文件不存在"
  exit 1
fi
